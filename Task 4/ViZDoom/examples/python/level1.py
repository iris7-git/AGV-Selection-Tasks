...
#  CRITICAL RULE: never mix MOVE_FORWARD=True with non-zero TURN_DELTA
#                 in the same tick — causes engine shake/wobble.
#  ROTATE state  → pure turn tick:    [F,F,F,F, delta]
#  FORWARD state → pure forward tick: [F,F,T,F, 0]
#  BACKUP state  → pure back tick:    [F,F,F,T, 0]
# ═══════════════════════════════════════════════════════════════════════════════

TURN_DELTA_COARSE = -3        # deg/tick — always positive (right turn only, full 360)
ALIGN_ENTER       = 5.0      # deg error to exit ROTATE → enter FORWARD
ALIGN_EXIT        = 15.0     # deg error to exit FORWARD → re-enter ROTATE (hysteresis)
ARRIVAL_RADIUS    = 35.0     # game-units: waypoint considered reached
DEADLOCK_STEPS    = 150      # ticks window for deadlock detection
DEADLOCK_MOVE_THR = 8.0      # min game-units moved in window to not be stuck
BACKUP_STEPS      = 25       # ticks of pure backward movement to escape deadlock
    ...
    def get_action(self, pos_x, pos_y, angle_deg):
        ...
        if dist < ARRIVAL_RADIUS:
            ...
            if self.wp_idx >= len(self.waypoints):
                ...
                return [False, False, False, False, 0]
            print(f"[NAV] wp {self.wp_idx-1} reached → heading to "
        ...
        if self.state == NavState.ROTATE:
            if abs(err) <= ALIGN_ENTER:
                self.state = NavState.FORWARD   # aligned → fall through
            else:
                return [False, False, False, False, TURN_DELTA_COARSE]
...
def _angle_diff(desired, current):
    """Always [0,360) — turn right only, no oscillation."""
    return (desired - current) % 360
...
if __name__ == "__main__":
    ...
    #The buttons you can use to make actions currently are:
    ...
    for ep in range(episodes):
        ...
        # ════════════════════════════════════════════════════════════════
        #  STAGE 1 — PATH PLANNING  (no movement)
        # ════════════════════════════════════════════════════════════════
        ...
        # Show planned path — press any key to begin movement
        ...
        # Calibration: nudge forward to get a second (pixel, world) sample
        ...
        for _ in range(CALIB_NUDGE_STEPS):
            ...
            game.make_action([False, False, True, False, 0])
            s2 = game.get_state()
            if s2 is None: break
        ...
        if not calib.is_ready():
            print("  [WARN] Calibration failed — skipping episode")
            continue
