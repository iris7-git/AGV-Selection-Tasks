...
# ── Tunable constants ─────────────────────────────────────────────────────────
TURN_DELTA_COARSE  = 3        # deg/tick rotating in place — always positive (one direction)
ALIGN_ENTER        = 5.0      # deg error: stop rotating, start moving forward
ALIGN_EXIT         = 15.0     # deg error: stop moving, go back to rotating
                              # Error is always in [0,360) — worst case 359° turn but no oscillation
    ...
    def get_action(self, pos_x, pos_y, angle_deg):
        ...
        if self.state == NavState.ROTATE:
            if abs(err) <= ALIGN_ENTER:
                ...
                # Fall through to FORWARD this same tick
            else:
                # Always positive delta — turn in one direction only
                delta = TURN_DELTA_COARSE
                return [False, False, False, False, delta]

        # ── FORWARD: pure move, zero turn delta ───────────────────────────
        # If heading drifted too far, switch back to ROTATE (hysteresis)
        if abs(err) > ALIGN_EXIT:
            self.state = NavState.ROTATE
            delta = TURN_DELTA_COARSE
            return [False, False, False, False, delta]

        # Heading is good — move forward, NO turn delta at all
...
def _angle_diff(desired, current):
    """Always positive [0,360) — agent turns in ONE direction only (always right).
    Worst case is 359° turn but eliminates left/right oscillation entirely."""
